...
do
   ...
   if [[ $count -le 0 || $force -gt 0 ]]; then     
      ...
      echo "/usr/local/bin/pacer_dirty_imager_${processor} ${base} -p _channel${fine_ch_str}_time000000_pol0 -n ${size} -f ${freq_mhz} -F ${fov} -w N -M ${obsID}.metafits -O ${fine_ch_str}${postfix}/ ${options} > ${fine_ch_str}${postfix}/out 2>&1"
      /usr/local/bin/pacer_dirty_imager_${processor} ${base} -p _channel${fine_ch_str}_time000000_pol0 -n ${size} -f ${freq_mhz} -F ${fov} -w N -M ${obsID}.metafits -O ${fine_ch_str}${postfix}/ ${options} > ${fine_ch_str}${postfix}/out 2>&1
   else
