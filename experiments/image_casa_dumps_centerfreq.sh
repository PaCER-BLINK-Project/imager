...
center_coarse_channel=133
if [[ -n "$2" && "$2" != "-" ]]; then
   center_coarse_channel=$2
fi
...
do
   ...
   count=`ls ${fine_ch_str}/dirty_image*_real.fits | wc -l`      
   if [[ $count -le 0 || $force -gt 0 ]]; then     
      mkdir -p ${fine_ch_str}
      echo "/usr/local/bin/pacer_dirty_imager_cpu ${base} -p _channel${fine_ch_str}_time000000_pol0 -n ${size} -f ${freq_mhz} -F ${fov} -w N -M ${obsID}.metafits -O ${fine_ch_str}${postfix}/ ${options} > ${fine_ch_str}/out 2>&1"
      /usr/local/bin/pacer_dirty_imager_cpu ${base} -p _channel${fine_ch_str}_time000000_pol0 -n ${size} -f ${freq_mhz} -F ${fov} -w N -M ${obsID}.metafits -O ${fine_ch_str}${postfix}/ ${options} > ${fine_ch_str}/out 2>&1
   else
