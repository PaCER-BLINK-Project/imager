...
def find_value_in_gpu( val_re, val_im, gpu_data, allow_conjugate=True, limit=0.0000001 ) :
   ...
   for x in range (0,x_size_gpu,2) : # step of 2 floats to save RE/IM :
     xx_re = gpu_data[ch,x]
...
debug=0
# fitsname="re.fits"
fitsname_re = "1276619416_20200619163000_vis_real_channel000_time000000_pol00.fits"
fitsname_im = "1276619416_20200619163000_vis_imag_channel000_time000000_pol00.fits"
...
hdu_mapping = pyfits.PrimaryHDU()
hdu_mapping.data = numpy.zeros( (n_ant,n_ant))
...
for ant1 in range(0,x_size) :
   for ant2 in range(0,y_size) :
      ...
      (found,x,gpu_re,gpu_im) = find_value_in_gpu( re, im, gpu_data, allow_conjugate=False )
      if found :
         sign_value=+1
         # if (gpu_im*im) < 0 :
         #   sign_value=-1
         print("MAPPING : found value (%.8f,%.8f) for antennas (%d,%d) at GPU index x = %d (%.8f,%.8f) sign = %d" % (re,im,ant1,ant2,x,gpu_re,gpu_im,sign_value))
         hdu_mapping.data[ant1,ant2] = int(x)
      else :
         print("WARNING : did not find value (%.8f,%.8f) for antennas (%d,%d) in GPU file -> trying conjugate" % (re,im,ant1,ant2))
         (found,x,gpu_re,gpu_im) = find_value_in_gpu( re, -im, gpu_data, allow_conjugate=False )

         if found :         
            hdu_mapping.data[ant1,ant2] = -int(x)
            print("\tFOUND CONJUGATE OK!!!")
         else : 
           print("!!! ERROR : did not find value (%.8f,%.8f) for antennas (%d,%d) in GPU file" % (re,im,ant1,ant2))    


hdu_mapping.writeto('mapping_xx.fits',clobber=True)
