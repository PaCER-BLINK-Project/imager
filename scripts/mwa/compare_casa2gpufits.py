...
def parse_options():
   ...
   parser.add_option('--gpufits','--gpufitsfile',dest="gpufitsfile",default="1276619416_20200619163000_gpubox24_00.fits", help="GPU FITS file [default %default]")
...
if __name__ == '__main__':         
   ...
   print("GPU FITS File        = %s" % (options.gpufitsfile))   
...
# data=tb.getcol(col) #This is pol x channel x antenna
   ...
   postfix= ( 'channel%03d_time%06d' % (options.channel,options.time_step))
   out_vis_refits=casams.replace('.ms', ('_vistest_real_%s.fits' % postfix))
   out_vis_imfits=casams.replace('.ms', ('_vistest_imag_%s.fits' % postfix))
   out_u_fits=casams.replace('.ms', ('_utest_%s.fits' % postfix))
   out_v_fits=casams.replace('.ms', ('_vtest_%s.fits' % postfix))
   out_w_fits=casams.replace('.ms', ('_wtest_%s.fits' % postfix))

   out_text_file = casams.replace('.ms', ('_vistest_%s.txt' % postfix))
   vis_file_re_f = open( out_text_file ,"w" )
   
   # Check GPU box FITS file:
   print("-------------------------------------------")
   print("DEBUG : GPU BOX FITS file:")
   print("-------------------------------------------")
   fits = pyfits.open(options.gpufitsfile)
   x_size=fits[0].header['NAXIS1']
   y_size=fits[0].header['NAXIS2']
   print("GPU BOX DATA shape = %d x %d" % (x_size,y_size))

   gpu_data = fits[0].data
   n_vis_to_print=100
   y = 0 
   for x in range (0,n_vis_to_print,2) : # step of 2 floats to save RE/IM :
      val_re = gpu_data[y,x]
      val_im = gpu_data[y,x+1]
      print("Vis[%d] = %.8f + j%.8f" % (int(x/2),val_re,val_im)) 
   print("-------------------------------------------")   
...
#      sys.exit(-1)
   ...
   for i1 in range(i,len(ant_data1)) : # starting from i which is the starting row of specified integration (see --time option)
      ...
      if curr_ant1 == curr_ant2 :
         print("AUTO %d-%d : %.8f + j%.8f ( %s )" % (curr_ant1,curr_ant2,numpy.real( data[options.polarisation,options.channel,i1] ),numpy.imag( data[options.polarisation,options.channel,i1] ),data[0,options.channel,i1]))
      ...
      if curr_ant1 >= prev_ant1 : 
         ...
         print("DEBUG saving visibility for antenna pair %d - %d = %.4f (%s)" % (ant_data1[i1],ant_data2[i1],numpy.real( data[0,options.channel,i1] ), flag ))
