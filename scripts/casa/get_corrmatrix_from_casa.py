...
def parse_options():
   ...
   parser.add_option('--flagged_antennas','--bad_antennas',dest="flagged_antennas",default=None, help="Coma separated list of flagged antennas [default %default]")
...
if __name__ == '__main__':         
   ...
   flagged_antennas = []
   if options.flagged_antennas is not None and len(options.flagged_antennas) > 0 :
   ...
   print("Number of flagged antennas = %d" % (len(flagged_antennas)))
   print("####################################################################################")
   ...
   ant_file = open("casa_antennas.txt","w")
...
#      position = Ant_positions[ant_idx]
      ...
      line = ("%s %.8f %.8f %.8f\n" % (Ant_names[ant_idx],position_x,position_y,position_z))
      ant_file.write( line )
...
# data=tb.getcol(col) #This is pol x channel x antenna
   ...
   out_vis_refits=casams.replace('.ms', ('_vis_real_%s_pol%d.fits' % (postfix,options.polarisation)))
   out_vis_imfits=casams.replace('.ms', ('_vis_imag_%s_pol%d.fits' % (postfix,options.polarisation)))
   out_u_fits=casams.replace('.ms', ('_u_%s_pol%d.fits' % (postfix,options.polarisation)))
   out_v_fits=casams.replace('.ms', ('_v_%s_pol%d.fits' % (postfix,options.polarisation)))
   out_w_fits=casams.replace('.ms', ('_w_%s_pol%d.fits' % (postfix,options.polarisation)))

   out_text_file = casams.replace('.ms', ('_vis_%s_pol%d.txt' % (postfix,options.polarisation)))
...
#      sys.exit(-1)
   ...
   for i1 in range(i,len(ant_data1)) : # starting from i which is the starting row of specified integration (see --time option)
      ...
      if curr_ant1 in flagged_antennas or curr_ant2 in flagged_antennas :
         print("WARNING : antenna %d or %d is flagged -> ignored" % (curr_ant1,curr_ant2))
         continue
