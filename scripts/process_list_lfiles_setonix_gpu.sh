...
#SBATCH --gres=gpu:1
#SBATCH --output=./process_all_lfiles_setonix_gpu.o%j
#SBATCH --error=./process_all_lfiles_setonix_gpu.e%j
#SBATCH --export=NONE
...
pacer_imager_path=/software/projects/director2183/msok/imager_branches/main/imager/build_gpu_20240109/pacer_imager_multi
if [[ -n "$7" && "$7" != "-" ]]; then
   pacer_imager_path="$7"
fi

modules="blink-imager-gpu/devel"
if [[ -n "$8" && "$8" != "-" ]]; then
   modules="$8"
fi
...
do
   ...
   if [[ ! -s ${tar_file} || $force -gt 0 ]]; then   
     ...
     echo "time ${pacer_imager_path} ${l_file_base} -f ${center_freq_mhz} -n 180  -Z -B 1 -O fits_gpu -t 0.100 -C 32 -j -1 -G $imager_options > out 2>&1"
     time ${pacer_imager_path} ${l_file_base} -f ${center_freq_mhz} -n 180  -Z -B 1 -O fits_gpu -t 0.100 -C 32 -j -1 -G $imager_options > out 2>&1 
